...
class StructureAuxiliaryLoss(nn.Module):
    """BCE-Dice segmentation with optional boundary and centerline supervision."""

    def __init__(self, segmentation_loss, boundary_weight=0.0, centerline_weight=0.0):
        super().__init__()
        self.segmentation_loss = segmentation_loss
        self.boundary_weight = float(boundary_weight)
        self.centerline_weight = float(centerline_weight)
        self.auxiliary_loss = BCEDiceLoss()

    def forward(self, segmentation_logits, mask_target, boundary_logits=None, centerline_logits=None, boundary_target=None, centerline_target=None):
        loss_seg = self.segmentation_loss(segmentation_logits, mask_target)
        loss_boundary = segmentation_logits.new_zeros(())
        loss_centerline = segmentation_logits.new_zeros(())

        if self.boundary_weight > 0.0:
            if boundary_logits is None or boundary_target is None:
                raise ValueError("Boundary auxiliary weight requires boundary logits and target")
            loss_boundary = self.auxiliary_loss(boundary_logits, boundary_target)

        if self.centerline_weight > 0.0:
            if centerline_logits is None or centerline_target is None:
                raise ValueError("Centerline auxiliary weight requires centerline logits and target")
            loss_centerline = self.auxiliary_loss(centerline_logits, centerline_target)

        total = loss_seg + self.boundary_weight * loss_boundary + self.centerline_weight * loss_centerline
        return total, loss_seg, loss_boundary, loss_centerline
