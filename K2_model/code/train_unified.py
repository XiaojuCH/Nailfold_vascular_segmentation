...
from models.compact_green_morphology import CompactGreenMorphologyAdapter
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="baseline",
        choices=["baseline", "ours", "prior_fusion", "soft_kd", "cgma"],
    )
    ...
    parser.add_argument("--cgma_prior", type=str, default="on", choices=["on", "off"])
    parser.add_argument("--cgma_auxiliary", type=str, default="on", choices=["on", "off"])
    parser.add_argument("--cgma_boundary_weight", type=float, default=0.10)
    parser.add_argument("--cgma_centerline_weight", type=float, default=0.10)
    parser.add_argument(
...
def save_per_image_metrics(path, model, loader, mode, joint_model):
    """Write val metrics for the selected checkpoint without touching test."""
    rows = []
    with torch.no_grad():
        for batch in loader:
            images = batch["image"].to(next(model.parameters()).device)
            ...
            metrics = per_image_metrics_from_logits(
                forward_for_logits(model, images, mode, joint_model),
                masks,
            )
            start = len(rows)
            for offset, values in enumerate(metrics):
                rows.append({
                    "filename": loader.dataset.filenames[start + offset],
                    **values,
                })
...
def main():
    ...
    if args.exp_name:
        ...
        exp_name = (
            f"{args.dataset}/cgma_prior{args.cgma_prior}_aux{args.cgma_auxiliary}"
        )
    else:
    ...
    if args.mode == "cgma":
        print(
            "[*] cgma: "
            f"prior={args.cgma_prior}, auxiliary={args.cgma_auxiliary}, "
            f"boundary_w={args.cgma_boundary_weight}, "
            f"centerline_w={args.cgma_centerline_weight}"
        )
    ...
    train_dataset = VesselDataset(
        ...
        structure_targets=args.mode == "cgma",
    )
    ...
    if args.mode in ["baseline", "prior_fusion", "soft_kd", "cgma"]:
        if args.mode == "prior_fusion":
            ...
            use_auxiliary = args.cgma_auxiliary == "on"
            model = CompactGreenMorphologyAdapter(
                segmentor,
                use_prior=args.cgma_prior == "on",
                use_auxiliary_heads=use_auxiliary,
            ).to(device)
        else:
        ...
        if args.mode == "soft_kd":
            ...
            criterion = StructureAuxiliaryLoss(
                segmentation_criterion,
                boundary_weight=args.cgma_boundary_weight if use_auxiliary else 0.0,
                centerline_weight=args.cgma_centerline_weight if use_auxiliary else 0.0,
            ).to(device)
        else:
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        ...
        if args.mode == "ours":
            ...
            log_file.write(f"CGMA Prior: {args.cgma_prior}\n")
            log_file.write(f"CGMA Auxiliary: {args.cgma_auxiliary}\n")
            log_file.write(f"CGMA Boundary Weight: {args.cgma_boundary_weight}\n")
            log_file.write(f"CGMA Centerline Weight: {args.cgma_centerline_weight}\n")
            log_file.write(f"Intensity Aug: {args.intensity_aug}\n")
        if args.pretrained:
        ...
        for epoch in range(args.epochs):
            ...
            for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{args.epochs} [Train]", leave=False):
                ...
                if args.mode in ["baseline", "prior_fusion"]:
                    ...
                    outputs, boundary_logits, centerline_logits = model(images)
                    boundary_targets = batch.get("boundary_target")
                    centerline_targets = batch.get("centerline_target")
                    if boundary_targets is not None:
                        boundary_targets = boundary_targets.to(device)
                    if centerline_targets is not None:
                        centerline_targets = centerline_targets.to(device)
                    loss, l_seg, l_boundary, l_centerline = criterion(
                        outputs,
                        masks,
                        boundary_logits=boundary_logits,
                        centerline_logits=centerline_logits,
                        boundary_target=boundary_targets,
                        centerline_target=centerline_targets,
                    )
                    train_seg += l_seg.item()
                    train_boundary += l_boundary.item()
                    train_centerline += l_centerline.item()
                elif args.mode == "soft_kd":
                    outputs = model(images)
            ...
            if args.mode in ["baseline", "prior_fusion"]:
                ...
                log_str = (
                    f"Ep {epoch + 1:03d} | Loss(Tot:{avg_loss:.3f} "
                    f"Seg:{train_seg / n_batches:.3f} "
                    f"Bnd:{train_boundary / n_batches:.3f} "
                    f"Skel:{train_centerline / n_batches:.3f}) | "
                    f"Dice: {avg_metrics['dice']:.4f} | HD95: {avg_metrics['hd95']:.2f}"
                )
                if diagnostic_text:
                    log_str += f" | CGMA({diagnostic_text})"
            else:
                n_batches = len(train_loader)
    ...
    if not os.path.exists(os.path.join(save_path, "best_model.pth")):
        raise RuntimeError("Training completed without saving a best_model.pth")
    load_torch_state_dict(model, os.path.join(save_path, "best_model.pth"), device, "best validation weight")
    model.eval()
    save_per_image_metrics(
        os.path.join(save_path, "val_per_image.csv"),
        model,
        val_loader,
        args.mode,
        args.joint_model,
