...
class VesselDataset(Dataset):
    def __init__(
        ...
        stems = [os.path.splitext(filename)[0] for filename in self.filenames]
        if len(stems) != len(set(stems)):
            raise ValueError(
                "Duplicate image filename stems are unsupported because soft targets are stored as '<stem>.npy'."
            )
